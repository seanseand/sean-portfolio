...
export function AsciiPanel({ n, color, title }: AsciiPanelProps) {
  ...
  return (
    <div className="relative min-h-[360px]">
      <div
        ref={panelRef}
        className="halftone tornreal relative w-full h-full min-h-[360px] bg-[var(--panel-bg)] bg-blend-multiply flex items-center justify-center cursor-crosshair"
      ...
      >
        ...
        />
        <div
          className={`absolute top-[10px] right-[12px] font-mono font-bold text-[9px] text-[rgba(255,255,255,.85)] transition-opacity duration-[250ms] pointer-events-none z-[2] ${dropHint ? 'opacity-100' : 'opacity-0'}`}
        >
          {dropHint ?? "DROP PHOTO ↴"}
        </div>
      </div>
